...
(function(root, factory) {
    root.when = root.when || require("./when");
...
//   ab.browserNotSupportedMessage = "Browser does not support WebSockets (RFC6455)";
    ...
    ab.deriveKey = function(secret, extra) {
        if (extra && extra.salt) {
            var CryptoJS = require("./CryptoJS.pbkdf2");
            var salt = extra.salt;
...
//                throw ab.browserNotSupportedMessage;
        ...
        self._websocket.addEventListener("message", function(e)
        {
            if (ab._debugws) {
                self._rxcnt += 1;
                console.log("WS Receive");
                Ti.API.info(self._wsuri + "  [" + self._session_id + "]");
            ...
            if (o[1] in self._calls)
            {
                if (o[0] === ab._MESSAGE_TYPEID_CALL_RESULT) {
                    ...
                    if (ab._debugrpc && dr._ab_callobj !== undefined) {
                        Ti.API.info("WAMP Call", dr._ab_callobj[2]);
                        Ti.API.info(dr._ab_tid);
                    ...
                    if (ab._debugrpc && de._ab_callobj !== undefined) {
                        Ti.API.info("WAMP Call", de._ab_callobj[2]);
                        Ti.API.info(de._ab_tid);
                        Ti.API.info("Arguments");
        ...
        self._websocket.addEventListener("open", function(e)
        {
            ...
            if (self._websocket.protocol !== ab._subprotocol) {
                ...
                if (typeof self._websocket.protocol === 'undefined') {
                    ...
                    //
                    if (ab._debugws) {
                        Ti.API.info("WS Warning");
                        Ti.API.info(self._wsuri);
        ...
        self._websocket.addEventListener("error", function(e)
        {
        });

        self._websocket.addEventListener("close", function(e)
        {
            if (ab._debugws) {
        ...
        // open connection
        self._websocket.open(wsuri);



        self.log = function() {
            if (self._options && 'sessionIdent' in self._options) {
                Ti.API.info("WAMP Session '" + self._options.sessionIdent + "' [" + self._session_id + "]");
            } else {
                Ti.API.info("WAMP Session " + "[" + self._session_id + "]");
            }
